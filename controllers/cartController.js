...
module.exports = {
    getItems: async (req, res) => {
        try {
            ...
            const items = await res.locals.sqlQuery(query, [res.locals.user.user_id]);
    ...
    addItem: async (req, res) => {
        try {
            ...
            const inventory_quantity = await res.locals.sqlQuery(inventory_query, [req.body.item_id]);
            if (req.body.quantity > inventory_quantity[0].quantity) {
                ...
                const items = await res.locals.sqlQuery(add_query, [res.locals.user.user_id, req.body.item_id, req.body.quantity || null, req.body.quantity || null]);
    ...
    deleteItem: async (req, res) => {
        try {
            ...
            const items = await res.locals.sqlQuery(delete_query, [res.locals.user.user_id, req.body.item_id]);
        ...
        try {
            ...
            switch (true) {
                ...
                default:
                    ...
                    await res.locals.sqlQuery(update_query, [req.body.quantity, res.locals.user.user_id, req.body.item_id]);
                    res.json({ success: true });
    ...
    placeOrder: async (req, res) => {
        ...
        try {
            ...
            await res.locals.sqlQuery(add_order_query, [res.locals.user.user_id, res.locals.user.user_id, req.body.phone, req.body.address, cur_date]);
            const order_id_query = "SELECT LAST_INSERT_ID() as id";
            const order_id = await res.locals.sqlQuery(order_id_query);
            ...
            const fetch_item_data = await res.locals.sqlQuery(fetch_item_data_query, [res.locals.user.user_id]);
            ...
            for (let i = 0; i < fetch_item_data.length; i++) {
                ...
                await res.locals.sqlQuery(add_order_items_query, [order_id[0].id, res.locals.user.user_id, fetch_item_data[i].price, fetch_item_data[i].quantity, fetch_item_data[i].item_id]);

            }

            const fetch_order_item_data_query = 'SELECT item_id, order_items.quantity  FROM order_items WHERE order_id =?'
            const fetch_order_item_data = await res.locals.sqlQuery(fetch_order_item_data_query, [order_id[0].id]);
            const update_query = 'UPDATE `items` SET quantity=quantity-? WHERE id =?';

            for (let i = 0; i < fetch_order_item_data.length; i++) {
                await res.locals.sqlQuery(update_query, [fetch_order_item_data[i].quantity, fetch_order_item_data[i].item_id]);
    ...
    ,
        clearCart: async (req, res) => {
            ...
            try {
                ...
                const items = await res.locals.sqlQuery(delete_query, [res.locals.user.user_id]);
